...
- **`plan.md`**: the approved plan. It holds the overview, architecture and design decisions, testing approach, and the `## Milestones & Tasks` checklist of tasks (`## Milestones & Phases` of phases, in a plan written before tasks), whose first unchecked `#### - [ ] Task:` (or `Phase`) heading is the current task. Read it with `{{command}} plan file read <plan_name>/plan.md`.
- **The plan's `context.md`**: the per-task technical detail. It holds one `### Task: <title>` section per task (`### Phase N.M:` per phase in an older plan) with the files to change, complexity and agent strategy. Read it with `{{command}} plan file read <plan_name>/context.md`.
