...
branches, and calls the matching `{{command}} design` command directly.
...
Before any branch that names a source, run `{{command}} design sources` to see the declared
sources and their locations. Choose only from the names it returns, never a name you inferred.
`{{command}} design list` shows what those sources already hold, including which documents carry a
...
3. Write it:
   ```
   {{command}} design author --data '{"source":"<name>","path":"<path>"}' --from .spektacular/tmp/<slug>.md
   ```
...
2. Write it:
   ```
   {{command}} design write --data '{"source":"<name>","path":"<path>"}' --from <path to their file>
   ```
...
3. Record the reference with `{{command}} design ref add` if a spec should point at it.
...
1. Read the current document: `{{command}} design read --data '{"source":"<name>","path":"<path>"}'`.
...
4. Rewrite it with `{{command}} design author`, exactly as in the author branch. For a document
...
`{{command}} design write` instead, which again stores the bytes exactly as supplied.
`{{command}} design list` tells the two apart: a document Spektacular authored reports its status
...
without being touched. `{{command}} design ref list --data '{"spec":"<spec>"}'` reports what a spec
already references and whether each reference resolves, and `{{command}} design ref remove` drops
one from both sides.
...
checkpoint, **do not invoke `{{command}} design author` or `{{command}} design write`**. Either
