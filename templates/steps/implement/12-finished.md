...
- The task that was completed, and the acceptance criteria ticked for it in `{{plan_path}}`.
- Any deviations recorded in the task's entry in the inline `{{changelog_section_name}}` section of `{{plan_path}}`.
...
- All tasks in `{{plan_path}}` under its milestones section have been checked off.
- Per-task implementation entries have been appended to the inline `{{changelog_section_name}}` section of `{{plan_path}}`.
...
- The tasks that were completed (read the `#### - [x] Task:` headings, or `#### - [x] Phase` headings in a plan written before tasks, from `{{plan_path}}`).
