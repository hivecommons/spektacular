...
- **`plan.md`**: the approved plan. It holds the overview, architecture and design decisions, testing approach, and the `## Milestones & Phases` checklist, whose first unchecked `#### - [ ] Phase` heading is the current phase. Read it with `go run . plan file read <plan_name>/plan.md`.
- **The plan's `context.md`**: the per-phase technical detail. It holds one `### Phase N.M:` section per phase with the files to change, complexity and agent strategy. Read it with `go run . plan file read <plan_name>/context.md`.
